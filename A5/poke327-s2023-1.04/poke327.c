...
static void print_map()
{
  ...
  printf("\n\n\n");
  printw("POKEMON --- GOTTA CATCH EM ALL!");
...
void win_show(WINDOW *win, char *label){
  ...
  print_in_middle(win, 1, 0, width, label, COLOR_PAIR(COLOR_WHITE));
}
...
void game_loop()
{
  ...
  panel_datas[0].hide = TRUE;
  panel_datas[1].hide = TRUE;
  panel_datas[2].hide = TRUE;
  ...
  while (play) {
    ...
    if (c == &world.pc) {
      ...
      int key = getch();
      switch (key){ 
        //move player character one cell north and west
        case 'y':
        case '7':
          if (move_cost[char_pc][world.cur_map->map[c->pos[dim_y] - 1][c->pos[dim_x] - 1]] != INT_MAX){
            ...
            world.cur_map->cmap[c->pos[dim_y] - 1][c->pos[dim_x] - 1] = c;
            
            c->next_turn += move_cost[char_pc][world.cur_map->map[c->pos[dim_y] - 1]
                                                            [c->pos[dim_x] - 1]];
            c->pos[dim_x]--;
            c->pos[dim_y]--;
        ...
        //move player character one cell north
        case 'k':
        case '8':
          if (move_cost[char_pc][world.cur_map->map[c->pos[dim_y] - 1][c->pos[dim_x]]] != INT_MAX){
            world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = NULL;
            world.cur_map->cmap[c->pos[dim_y] - 1][c->pos[dim_x]] = c;
        ...
        //move player character one cell north and east
        case 'u':
        case '9':
          if (move_cost[char_pc][world.cur_map->map[c->pos[dim_y] - 1][c->pos[dim_x] + 1]] != INT_MAX){
            world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = NULL;
        ...
        //move player character one cell west
        case 'h':
        case '4':
          if (move_cost[char_pc][world.cur_map->map[c->pos[dim_y]][c->pos[dim_x] - 1]] != INT_MAX){
            world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = NULL;
            world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x] - 1] = c;
        ...
        //move player character one cell east
        case 'l':
        case '6':
          if (move_cost[char_pc][world.cur_map->map[c->pos[dim_y]][c->pos[dim_x] + 1]] != INT_MAX){
            world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = NULL;
        ...
        //move player character one cell south and west
        case 'b':
        case '1':
          if (move_cost[char_pc][world.cur_map->map[c->pos[dim_y] + 1][c->pos[dim_x] - 1]] != INT_MAX){
            world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = NULL;
        ...
        //move player character one cell south
        case 'j':
        case '2':
          if (move_cost[char_pc][world.cur_map->map[c->pos[dim_y] + 1][c->pos[dim_x]]] != INT_MAX){
            world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = NULL;
        ...
        //move player character one sell south east
        case 'n':
        case '3':
          if (move_cost[char_pc][world.cur_map->map[c->pos[dim_y] + 1][c->pos[dim_x] + 1]] != INT_MAX){
            world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = NULL;
            ...
            // if (world.cur_map->cmap[c->pos[dim_y] + 1][c->pos[dim_x] + 1]->symbol == HIKER_SYMBOL || world.cur_map->cmap[c->pos[dim_y] + 1][c->pos[dim_x] + 1]->symbol == RIVAL_SYMBOL || world.cur_map->cmap[c->pos[dim_y] + 1][c->pos[dim_x] + 1]->symbol == SWIMMER_SYMBOL
            // || world.cur_map->cmap[c->pos[dim_y] + 1][c->pos[dim_x] + 1]->symbol == WANDERER_SYMBOL || world.cur_map->cmap[c->pos[dim_y] + 1][c->pos[dim_x] + 1]->symbol == SENTRY_SYMBOL
            // || world.cur_map->cmap[c->pos[dim_y] + 1][c->pos[dim_x] + 1]->symbol == PACER_SYMBOL || world.cur_map->cmap[c->pos[dim_y] + 1][c->pos[dim_x] + 1]->symbol == EXPLORER_SYMBOL){
            //   move(22, 1);
            //   printw("BATTLE TIME: press Escape to exit");
            //   int inBattle = 1;
            //   while(inBattle){
            //     getch();
            //     if(key == 27){
        ...
        //Attempt to enter PokeMart or Center
        case '>':
          if (world.cur_map->map[c->pos[dim_y]][c->pos[dim_x]] == ter_center){
            ...
            while (canPrint){
              move(23, 1);
              printw("This is the Center:  Press < to Exit");
              int key;
              key = getch();
          ...
          if (world.cur_map->map[c->pos[dim_y]][c->pos[dim_x]] == ter_mart){
            ...
            while (canPrint){
              move(23, 1);
              printw("This is the Mart:  Press < to Exit");
              int key;
              key = getch();
        ...
        for (i = 0; i < MAP_X; i++){
          for (j = 0; j < MAP_Y; j++){
            if (world.cur_map->cmap[j][i]){
              if (world.cur_map->cmap[j][i]->symbol == HIKER_SYMBOL){
                myNpcs[curSpot].type = char_hiker;
                myNpcs[curSpot].name = "Hiker";
                myNpcs[curSpot].x_pos = i;
                myNpcs[curSpot].y_pos = j;
                curSpot++;
                size++;
              }
              if (world.cur_map->cmap[j][i]->symbol == RIVAL_SYMBOL){
                myNpcs[curSpot].type = char_hiker;
                myNpcs[curSpot].name = "Rival";
                myNpcs[curSpot].x_pos = i;
                myNpcs[curSpot].y_pos = j;
                curSpot++;
                size++;
              }
              if (world.cur_map->cmap[j][i]->symbol == SWIMMER_SYMBOL){
                myNpcs[curSpot].type = char_hiker;
                myNpcs[curSpot].name = "Swimmer";
                myNpcs[curSpot].x_pos = i;
                myNpcs[curSpot].y_pos = j;
                curSpot++;
                size++;
              }
              if (world.cur_map->cmap[j][i]->symbol == SENTRY_SYMBOL){
                myNpcs[curSpot].type = char_hiker;
                myNpcs[curSpot].name = "Sentry";
                myNpcs[curSpot].x_pos = i;
                myNpcs[curSpot].y_pos = j;
                curSpot++;
                size++;
              }
              if (world.cur_map->cmap[j][i]->symbol == WANDERER_SYMBOL){
                myNpcs[curSpot].type = char_hiker;
                myNpcs[curSpot].name = "Wanderer";
                myNpcs[curSpot].x_pos = i;
                myNpcs[curSpot].y_pos = j;
                curSpot++;
                size++;
              }
              if (world.cur_map->cmap[j][i]->symbol == EXPLORER_SYMBOL){
                myNpcs[curSpot].type = char_hiker;
                myNpcs[curSpot].name = "Explorer";
                myNpcs[curSpot].x_pos = i;
                myNpcs[curSpot].y_pos = j;
                curSpot++;
                size++;
              }
              if (world.cur_map->cmap[j][i]->symbol == PACER_SYMBOL){
                myNpcs[curSpot].type = char_hiker;
                myNpcs[curSpot].name = "Pacer";
                myNpcs[curSpot].x_pos = i;
                myNpcs[curSpot].y_pos = j;
                curSpot++;
                size++;
        ...
        i = 0;
          move(22, 1);
          int inMenu = 1;
          printw("This %s NPC is at Position %d %d!      ", myNpcs[i].name, myNpcs[i].x_pos, myNpcs[i].y_pos);
          ++i;
          move(23, 1);
          printw("This %s NPC is at Position %d %d!      ", myNpcs[i].name, myNpcs[i].x_pos, myNpcs[i].y_pos);
          ++i;
        ...
        //keep player character still for turn
        case ' ':
        case '5':
        case '.':
          c->next_turn += move_cost[char_pc][world.cur_map->map[c->pos[dim_y]]
