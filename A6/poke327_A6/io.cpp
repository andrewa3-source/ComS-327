...
void io_handle_input(pair_t dest)
{
  ...
  do {
    ...
    case 'f':
      ...
      move(0,0);
      printw("Where is the x location you would like to go to: ");
      refresh();
        for (i = 0; i < 10; i++) {
          input[i] = getch();  // Read a single character from the user
          if (input[i] == '\n') {  // Stop reading input if the user presses "Enter"
              break;
          }
          addch(input[i]);  // Display the user's input on the screen
      }
      input[i] = '\0';  // Add a null terminator to the end of the input string
      ...
      printw("                                                        ");
      move(0,0);
      printw("Where is the y location you would like to go to: ");
      refresh();
        for (i = 0; i < 10; i++) {
          input[i] = getch();  // Read a single character from the user
          if (input[i] == '\n') {  // Stop reading input if the user presses "Enter"
      ...
      dest[dim_y] = numy;
      world.cur_idx[dim_x] = dest[dim_x];
      world.cur_idx[dim_y] = dest[dim_y];
      world.cur_map->cmap[world.pc.pos[dim_y]][world.pc.pos[dim_x]] = NULL;
      // world.pc.pos[dim_x] = 40;
      // world.pc.pos[dim_y] = 10;
      new_map(1);
      io_display();
      refresh();
      turn_not_consumed = 1;
