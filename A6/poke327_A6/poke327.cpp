...
static int smooth_height(map_t *m)
{
  ...
  for (i = 1; i < 255; i += 20) {
    ...
    height[y][x] = i;
    if (i == 1) {
      head = tail = static_cast<queue_node *>(malloc(sizeof (*tail)));
    } else {
      tail->next = static_cast<queue_node *>(malloc(sizeof (*tail)));
      tail = tail->next;
    }
    tail->next = NULL;
    tail->x = x;
...
void new_hiker()
{
  ...
  world.cur_map->cmap[pos[dim_y]][pos[dim_x]] = c = static_cast<character *>(malloc(sizeof (*c)));
  c->npc = static_cast<npc_t *>(malloc(sizeof (*c->npc)));
...
void init_pc()
{
  ...
  world.pc.symbol = '@';
  world.pc.pc = static_cast<pc_t *>(malloc(sizeof (*world.pc.pc)));
  world.pc.npc = NULL;
...
void place_pc()
{
  ...
  if ((c = static_cast<character *>(heap_peek_min(&world.cur_map->turn)))) {
...
int new_map(int teleport)
{
  ...
  world.cur_map                                             =
    ...
    static_cast<map_t *>(malloc(sizeof (*world.cur_map)));
...
void game_loop()
{
  ...
  while (!world.quit) {
    c = static_cast<character *>(heap_remove_min(&world.cur_map->turn));
