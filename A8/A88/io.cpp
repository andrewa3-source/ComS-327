...
void makePokemon(pokemon_obj &wild){
  int r;
  int i;
  // pokemon_obj wild;
  
  r = (rand() % 1092) + 1;
    wild.identifier = pokemon[r].identifier;

    int x = world.cur_idx[dim_x] - 200;
    
    int y = world.cur_idx[dim_y] - 200;
    ...
    if (x + y < 200){
      wild.level = rand() % ((x + y  + 2) / 2) + 1; 
    }
    else {
      int yes = (100 - (x + y - 200 + 2) / 2);
      wild.level = ((rand() % yes) + (x + y - 200 + 2) / 2);
    }

    int range[1000];
    int j = 0;
    for (i = 0; i < 528284; ++i){
      if ((pokemon_moves[i].pokemon_id == pokemon[r].species_id) && (pokemon_moves[i].pokemon_move_method_id == 1)){
    ...
    int ch = rand() % j;
    wild.moves_int[0] = pokemon_moves[range[ch]].move_id;
    ch = rand() % j;
    wild.moves_int[1] = pokemon_moves[range[ch]].move_id;

    wild.moves_name[0] = moves[wild.moves_int[0]].identifier;
    wild.moves_name[1] = moves[wild.moves_int[1]].identifier;

    for (i = 1; i < 6554; ++i){
      if (pokemon_stats[i].pokemon_id == pokemon[r].id){
        int j = i;
        wild.health = pokemon_stats[j].base_stat;
        wild.damage = pokemon_stats[j+1].base_stat;
        wild.defense = pokemon_stats[j + 2].base_stat;
        wild.special_attack = pokemon_stats[j + 3].base_stat;
        wild.special_defense = pokemon_stats[j + 4].base_stat;
        wild.speed = pokemon_stats[j+5].base_stat;
      }
    }
...
static void io_list_pokemon(){
  ...
  snprintf(s[0], 40, "Choose a starting Pokemon \n");
  mvprintw(4, 19, " %-40s  ", *s);
  snprintf(s[0], 40, "Option 1: %s", one.identifier);
  mvprintw(5, 19, " %-40s ", *s);
  snprintf(s[0], 40, "Option 2: %s", two.identifier);
  mvprintw(6, 19, " %-40s ", *s);
  snprintf(s[0], 40, "Option 3: %s", three.identifier);
  mvprintw(7, 19, " %-40s ", *s);
  
  snprintf(s[0], 40, "Select your option with numer keys");
  mvprintw(9, 19, " %-40s ", *s);
  snprintf(s[0], 40, "Click Twice to Confirm");
  mvprintw(10, 19, " %-40s ", *s);
  
  // mvprintw(6, 19, " %-40s ", *s[2]);
  // mvprintw(7, 19, " %-40s ", *s[3]);
  char key;
  key = getch();
  while(true){
  switch (key) {
    case '1':
      world.pc.myPoke[0] = one;
      return;
    case '2':
      world.pc.myPoke[0] = two;
      return;
    case '3':
      world.pc.myPoke[0] = three;
      return;
    default:
      return;
    }
...
void io_pokemon_encounter(){
  ...
  if(r == 1){
    ...
    io_display();
    mvprintw(22, 0, "A level %d %s has appeared!                            ", wild.level, wild.identifier);
    mvprintw(23, 0,  "- moves: %s, and %s. hp: %d, attack: %d!              ", wild.moves_name[0], wild.moves_name[1], wild.health, wild.damage);
