...
int pokemon::get_move_power(int i) const
{
  if (i < 4 && move_index[i]) {
    if (moves[move_index[i]].power == INT_MAX){
      return 1;
    }
...
int pokemon::get_move_priority(int i) const
{
  if (i < 4 && move_index[i]) {
    if (moves[move_index[i]].priority != INT_MAX){
      return moves[move_index[i]].priority != INT_MAX;
    }
  }
  return 0;
}
