...
npc* new_gym_trainer(){
  ...
  pos[dim_x] = 100;
  pos[dim_y] = 100;
  world.cur_map->cmap[pos[dim_x]][pos[dim_y]] = c;
  c->mtype = move_sentry;
  c->ctype = char_gym_trainer;
  c->symbol = SENTRY_SYMBOL;
  c->defeated = 0;
  c->next_turn = 0;
  c->seq_num = world.char_seq_num++;
  //heap_insert(&world.cur_map->turn, c);
  //assign pokemon
  c->buddy[0] = new class pokemon(30);
  c->buddy[0]->hp = c->buddy[0]->get_hp();
  c->buddy[1] = new class pokemon(25);
  c->buddy[1]->hp = c->buddy[1]->get_hp();
  c->buddy[2] = new class pokemon(32);
  c->buddy[2]->hp = c->buddy[2]->get_hp();
  c->buddy[3] = new class pokemon(40);
  c->buddy[3]->hp = c->buddy[3]->get_hp();
  c->buddy[4] = new class pokemon(38);
  c->buddy[4]->hp = c->buddy[4]->get_hp();
  c->buddy[5] = new class pokemon(26);
  c->buddy[5]->hp = c->buddy[5]->get_hp();
